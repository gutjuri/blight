...
opts = 
  ...
  <$> 
    subparser
      ( command "show" (info (pure (Show Current)) (progDesc "Show current display brightness"))
     <> command "max"  (info (pure (Show Max)) (progDesc "Show maximum display brightness"))
     <> command "set"  (info (Set <$> argument auto (metavar "VALUE")) (progDesc "Set display brightness"))
     <> command "inc"  (info (Inc <$> argument auto (metavar "VALUE")) (progDesc "Increase display brightness"))
     <> command "dec"  (info (Dec <$> argument auto (metavar "VALUE")) (progDesc "Decrease display brightness")) 
      )
    <*> switch
      ( long "relative"
     <> short 'r'
     <> help "Using relative display brighness (as oppossed to the internal representation)"
      )
    <*> strOption
...
desc :: InfoMod a
desc = ( fullDesc
      <> progDesc "Manipulate display brightness"
      <> header "blight command line tool (c) 2019 Juri Dispan" )
...
setBrightness val maxB fp = let newBrightness = max 0 $ min maxB val in
                              writeFile fp $ show newBrightness ++ "\n"
...
main = do
  ...
  case getAction cfg of
    Show Current -> join $ print <$> fromInternal' <$> getBrightness (brightnessFile cfg)
    Show Max     -> putStr $ show maxBrightness
    Set val      -> setBrightness (toInternal' val) maxBrightness (brightnessFile cfg)
    Inc val      -> adjustBrightness (toInternal' val) maxBrightness (brightnessFile cfg)
    Dec val      -> adjustBrightness (toInternal' (-val)) maxBrightness (brightnessFile cfg)
